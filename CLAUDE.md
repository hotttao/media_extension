...
插件 → POST /v1/single/jimeng-image {styleImageId, sceneId}
     ...
     → 插件 claim 时 to_public_dict() 有 platform="jimeng_image"
```
...
platform="jimeng_image"→ runJimengImageJob (jimeng.jianying.com)
platform="jimeng_video"→ runJimengVideoJob (jimeng.jianying.com)
```
...
class Job:
    ...
    platform: str | None = None       # gpt / jimeng_image / jimeng_video
...
### kind 命名规范
- 使用 **kebab-case**：`jimeng-image`、`jimeng-video`、`first-frame-image`、`style-image`、`model-image`
- 不要用下划线：`jimeng_image`（下划线是 platform 值，不是 kind 值）
